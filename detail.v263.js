/* OkObserver · detail.v263.js · v2.7.2
   Media-first (poster → click-to-play YT/Vimeo; FB post plugin inline),
   removes title blue box, and shows author + pretty date.
*/
const API_BASE = (window.OKO_API_BASE || 'https://okobserver-proxy.bob-b5c.workers.dev/wp/v2').replace(/\/+$/, '');
...
// Find first video/post URL (Vimeo, YouTube, Facebook post/video)
function extractVideoURL(html=''){
  const unwrap = html.replace(/&amp;/g,'&');
  const d = document.createElement('div'); d.innerHTML = unwrap;

  // 1) explicit links inside content
  for(const a of d.querySelectorAll('a[href]')){
    ...
    if(/facebook\.com\/[^/]+\/videos\/\d+/.test(href) || /facebook\.com\/[^/]+\/posts\/\d+/.test(href)) return href;
  }
  // 2) iframes
...
function normalizePlayer(url){
  ...
  // FB post or video: use post plugin URL so FB decides rendering
...
export default async function renderDetail(app, idParam){
  ...
  if (poster && embed && embed.type !== 'facebook') {
    // Poster first → click swaps to player
...
const __once = 'oko-detail-scope-v272';
if (!document.getElementById(__once)) {
  ...
  /* kill inherited title backgrounds/boxes */
