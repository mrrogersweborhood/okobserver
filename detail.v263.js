/* OkObserver · detail.v263.js · v2.6.8
   Fixes:
   - Title blue box removed (scoped CSS with high specificity + !important)
   - Author + pretty date always rendered under title
   - Media-first layout (featured image or embed)
   - FB/Vimeo/YouTube detection
   - WP empty-wrapper cleanup + first-paragraph de-indent
*/
...
/* detect first video/post URL (Vimeo, YouTube, Facebook post/video) */
...
function normalizePlayer(url){
  ...
  // FB post or video: use post plugin URL so FB decides rendering (video posts work here too)
...
/* ---------- scoped overrides (kill title blue box; keep media sensible) ---------- */
const __once = 'oko-detail-scope-v268';
if (!document.getElementById(__once)) {
  ...
  /* Hard reset for title blue box (high specificity + !important) */
