/* OkObserver · detail.v263.js · v2.6.7 (media width + title reset + author meta + FB/Vimeo/YT)
   - Media first, constrained and centered (max-width: 900px; fluid on mobile)
   - Title hard-reset (no background, no box)
   - Author + pretty date always visible under title
   - Back to Posts buttons (top & bottom)
   - Facebook post/video, Vimeo, YouTube handling
   - Whitespace cleanup, first-paragraph de-indent
...
// detect first video/post URL (Vimeo, YouTube, Facebook)
function extractVideoURL(html=''){
  ...
  for(const a of d.querySelectorAll('a[href]')){
    ...
    if(/facebook\.com\/.+\/videos\/\d+/.test(href) || /facebook\.com\/[^/]+\/posts\//.test(href)) return href;
  }
...
function normalizePlayer(url){
  ...
  // FB post or video: use post plugin URL so FB decides rendering (video post works here too)
...
export default async function renderDetail(app, idParam){
  ...
  mount.innerHTML = `
    <article class="post-detail">
      ...
      <div class="oko-actions-bottom">${backButtonHTML()}</div>
    </article>
  `;
  ...
  try{
    ...
    mount.querySelector('.post-content').innerHTML = `<p class="error" style="color:#b00">Failed to load post.</p>`;
    return;
  }
  ...
  if (poster && embed && embed.type !== 'facebook') {
    ...
    if (embed.type === 'facebook') {
      // FB often blocks inline autoplay; use official post plugin iframe directly
    ...
    // no media → remove wrapper to avoid big gap
    $media.remove();
  }
...
/* ---------- scoped detail styles (safe & override hostile globals) ---------- */
const __once = 'oko-detail-scope-v267';
if (!document.getElementById(__once)) {
  ...
  .oko-actions-bottom{ margin-top: 1.1rem; }

  /* Media wrapper centered and constrained */
  .post-media{
  ...
  .oko-video-poster__play{
    ...
    background:#1e63ff;color:#fff;box-shadow:0 6px 16px rgba(0,0,0,.22);cursor:pointer;z-index:3
  }
  ...
  .oko-video-embed{
    position:relative;
    padding-top:56.25%;
    border-radius:12px;overflow:hidden;background:#000;margin:0;
  }
  .oko-video-embed iframe{position:absolute;inset:0;width:100%;height:100%}

  /* Fallback single image (featured) */
  .oko-detail-img{
    display:block;width:100%;height:auto;border-radius:12px;
    box-shadow:0 1px 3px rgba(0,0,0,.06)
  }

  /* Title reset: kill any inherited blue box/background/border/padding */
