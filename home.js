...
function formatDate(iso){
  try{ const d=new Date(iso); return d.toLocaleDateString(undefined,{year:"numeric",month:"long",day:"numeric"}); }
  catch{ return iso; }
}
...
export async function renderHome(){
  ...
  let items=[];
  try{ items = await fetchLeanPostsPage(1,6); }
  catch(e){ grid.innerHTML = `<p>Failed to load posts. ${e.message}</p>`; return; }
  grid.innerHTML = items.map(p=>{
    ...
    return `<article class="card">
      <a class="thumb-wrap" href="#/post/${p.id}">${img?`<img class="thumb" src="${img}" alt="">`:""}</a>
      <div class="card-body">
        <h3 class="title"><a href="#/post/${p.id}">${title}</a></h3>
