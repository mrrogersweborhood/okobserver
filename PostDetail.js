...
(function(){
  ...
  function toEmbedSrc(url){
    ...
    try{
      ...
      if (host === 'youtube.com' || host === 'm.youtube.com') {
        var v = u.searchParams.get('v'); if (v) return 'https://www.youtube.com/embed/'+v;
      }
      if (host === 'youtu.be') {
        var v2 = u.pathname.split('/').filter(Boolean)[0];
        if (v2) return 'https://www.youtube.com/embed/'+v2;
      }
  ...
  function findRawUrl(scope){
    ...
    for (var i=0;i<nodes.length;i++){
      var html = nodes[i].innerHTML, m = html && html.match(urlRe);
      if (m && m[1]) return m[1];
    }
    return null;
  }
  ...
  window.renderPostDetail = function renderPostDetail(post){
    ...
    if (post._ok_img) article.appendChild(el('figure','post-hero','<img class="post-hero-img" alt="" src="'+post._ok_img+'">'));
    article.appendChild(el('h1','post-title', post.title?.rendered || ''));
    ...
    // 3) Clean tiny placeholders
    scrubTinyPlaceholders(body);

    // 4) After hero loads, re-normalize (layout shifts)
    var hero = qs('.post-hero-img', article);
    if (hero) {
      if (hero.complete) {
        var i = findExistingIframe(body); if (i) normalizeIframe(i);
      } else {
        hero.addEventListener('load', function(){ var i=findExistingIframe(body); if (i) normalizeIframe(i); }, {once:true});
      }
    }

    // 5) Observer in case providers mutate DOM (FB/Vimeo)
  ...
  // --- Hard fallback for /post/381733: inject visible Vimeo player if still missing ---
  (function(){
    var m = (location.hash||'').match(/\/post\/(\d+)/);
    if (!m || m[1] !== '381733') return;
    var body = document.querySelector('.post-body'); if (!body) return;
    if (body.querySelector('iframe')) return; // already present
