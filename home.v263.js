// Home view — blue titles, date in meta, excludes "Cartoon", robust infinite scroll
const API_BASE = window.OKO_API_BASE;

/* helpers */
function stripHtml(html){const d=document.createElement("div");d.innerHTML=html||"";return d.textContent||"";}
async function apiFetchJson(url){const r=await fetch(url);if(!r.ok)throw new Error(`API Error ${r.status}`);const j=await r.json();return{json:j,headers:r.headers};}
...
async function getCartoonCategoryId(){
  ...
  try{
    ...
    const hit=Array.isArray(cats)?cats.find(c=>/cartoon/i.test(c?.slug||"")||/cartoon/i.test(c?.name||"")):null;
    CARTOON_CAT_ID=hit?hit.id:0;
  }catch{ CARTOON_CAT_ID=0; }
...
async function fetchPosts(page=1, perPage=9){
  ...
  const url=`${API_BASE}/posts?status=publish&per_page=${perPage}&page=${page}&_embed=1&orderby=date&order=desc${exclude}`;
