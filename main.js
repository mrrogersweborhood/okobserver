/* 🟢 main.js */
/* OkObserver main.js — v=2025-11-06SR1-perfSWR1-hotfix3p
   Changes (hotfix3p):
   - Detail view image is only inserted AFTER it fully loads (no favicon/placeholder flash).
   - No change to summary cards or other behavior.
*/
(function(){
  "use strict";

  var VER   = (window.__OKO__ && window.__OKO__.VER) || "2025-11-06SR1-perfSWR1-hotfix3p";
  ...
  function cardFromPost(post){
    ...
    title.style.color = "#1E90FF"; // OkObserver blue
  ...
  // Featured image picker (summary + detail)
  ...
  async function renderDetail(id){
    ...
    // 1) Featured image FIRST, but only insert after it's fully loaded (prevents any favicon/placeholder flash)
    var mediaURL = pickFeaturedImage(post);
    if (mediaURL){
      var img = new Image();
      img.className = "oo-media";
      img.decoding = "async";
      img.loading = "eager"; // show as soon as possible once loaded
      img.alt = textFromHTML(post.title && post.title.rendered || "");
      // Pre-load off-DOM:
      img.addEventListener("load", function(){ c.insertBefore(img, c.firstChild || null); }, { once:true });
      img.addEventListener("error", function(){ /* skip image silently on error */ }, { once:true });
      img.src = mediaURL; // start loading AFTER handlers are set
    }

    // 2) Title (OkObserver blue)
    var h = ce("h1");
