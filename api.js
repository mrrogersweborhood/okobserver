...
// v=2.3.3 (argument-safe: handles AbortSignal passed as 2nd arg)
...
function readCached(key) { try { return ss.getItem(key) || null; } catch { return null; } }
function writeCached(key, val) { try { ss.setItem(key, val); } catch {} }
...
// Alias to match existing imports in home.js
export async function getCartoonCategoryId(/* signal (ignored) */) {
...
/* ============== Argument normalization =================
   Accepts legacy calls like:
     fetchPostsPage(page, controller.signal)
     fetchLeanPostsPage(page, controller.signal)
   and modern:
     fetchPostsPage(page, perPage, { signal, excludeCartoon })
*/
function normPostsArgs(page, perPage, opts) {
  ...
  // If 2nd arg is a number, it's perPage; else it's opts/signal
  if (typeof perPage === "number" && isFinite(perPage)) {
    pp = perPage;
  } else if (perPage) {
    // legacy: perPage is actually an AbortSignal
...
// Lean variant kept for compatibility; also argument-safe
