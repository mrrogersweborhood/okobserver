// app.js — OkObserver v1.53 (dupe-filter + paragraph-align + scroll-to-top)
...
(() => {
  ...
  // manual restoration so we control list back-scroll
  ...
  // ---- tiny API cache (read-through) ----
  const apiCache = new Map();
  function k(u){return`__api:${u}`;} function km(u){return`__api:${u}:meta`;}
  function getCachedJSON(u){
    if(apiCache.has(u)) return apiCache.get(u);
    try{ const raw=sessionStorage.getItem(k(u));
      if(raw){ const val=JSON.parse(raw); apiCache.set(u,val); return val; } }catch{}
    return null;
  }
  function setCachedJSON(u,d,m){
    apiCache.set(u,d);
    try{ sessionStorage.setItem(k(u), JSON.stringify(d)); }catch{}
    if(m){ try{ sessionStorage.setItem(km(u), JSON.stringify(m)); }catch{} }
  }
  ...
  // detail summary cache for optimistic shell
  ...
  // about (minimal — live page used previously)
  ...
  // detail with optimistic shell + scroll-to-top
  async function renderPost(id){
    // always start at top when entering detail
