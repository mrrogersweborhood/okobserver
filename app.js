/* app.js — OkObserver (monolithic build) — v1.60.0
   Features kept: infinite scroll (IO + scroll fallback), stable return from detail
   (restores scroll + cursor/anchor), first-paragraph normalization, entity decode,
   safe external links, embed fallbacks, only bottom “Back to posts” button.
*/
(function () {
  "use strict";

  // -----------------------------
  // Version & simple logger
  // -----------------------------
  ...
  function saveHomeCache() {
    try {
      sessionStorage.setItem("__okCache", JSON.stringify(stateForSave(state)));
    } catch {}
  }
  (function rehydrate() {
    try {
  ...
  function showError(message) {
    ...
    div.innerHTML =
      '<button class="close" aria-label="Dismiss">×</button>' + msg;
    host.prepend(div);
  }
  ...
  function ordinalDate(iso) {
    ...
    const day = d.getDate();
    const suf = (n) =>
      n > 3 && n < 21 ? "th" : (["th", "st", "nd", "rd"][Math.min(n % 10, 4)] || "th");
    return (
  ...
  function transformEmbeds(root) {
    ...
    root
      .querySelectorAll(".wp-block-embed__wrapper, .wp-block-embed")
      .forEach((box) => {
        ...
        if (href) {
          const provider = href.includes("vimeo.com")
            ? "Vimeo"
          ...
          fallback.innerHTML =
            '<div>Video can’t be embedded here.</div><a class="btn" href="' +
            href +
            '" target="_blank" rel="noopener">Watch on ' +
            provider +
            "</a>";
  ...
  // Controllers for aborting fetches
  const controllers = { listAbort: null, detailAbort: null };
  // -----------------------------
  // Home grid rendering
  // -----------------------------
  ...
  function hideLoader() {
    const ld = document.getElementById("infiniteLoader");
  ...
  function renderGridFromPosts(posts, append = false) {
    ...
    if (state._io && typeof state._io.observe === "function") {
      if (state._sentinel && state._sentinel !== s) {
        try {
          state._io.unobserve(state._sentinel);
        } catch {}
      }
      state._io.observe(s);
      state._sentinel = s;
  ...
  async function loadNextPage() {
    ...
    if (controllers.listAbort) {
      try {
        controllers.listAbort.abort();
      } catch {}
    }
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        signal: controllers.listAbort.signal,
      });
  ...
  function ensureInfiniteScroll() {
    ...
    if (state._io && typeof state._io.observe === "function") {
      if (state._sentinel && state._sentinel !== sentinel) {
        try {
          state._io.unobserve(state._sentinel);
        } catch {}
      }
    ...
    state._io = io;
    state._sentinel = sentinel;
    state._ioAttached = true;
  }
  async function renderHome() {
    if (!app()) return;

    // Fast restore path when returning from detail
      ...
      (async () => {
        try {
          ...
          if (targetY > 0) window.scrollTo(0, targetY);
            ...
            const el = document.querySelector(`[data-id="${state.scrollAnchorPostId}"]`);
          ...
          if (targetY > 0) window.scrollTo(0, targetY);
            ...
            const el2 = document.querySelector(`[data-id="${state.scrollAnchorPostId}"]`);
    ...
    try {
      ...
      try {
        ...
        if (!res2.ok) {
          ...
          const err = new Error(
            `API Error ${res2.status}${res2.statusText ? `: ${res2.statusText}` : ""}`
          );
          err.details = text?.slice(0, 300);
          throw err;
        }
      ...
      if (!posts.length) {
        app().innerHTML = "";
        showError("No posts returned from the server.");
  ...
  async function renderPost(id) {
    renderPostShell();
    if (controllers.detailAbort) {
      try {
        controllers.detailAbort.abort();
      } catch {}
    }
    ...
    try {
      const res = await fetch(url, {
        headers: { Accept: "application/json" },
        signal: controllers.detailAbort.signal,
      });
