// app.js — OkObserver (v1.33.7-lite)
const APP_VERSION = "v1.33.7-lite";
...
(() => {
  ...
  const NEWSMAKERS_CAT = "newsmakers";
  ...
  const inCategory = (p, name) => {
    const cats = p?._embedded?.["wp:term"]?.[0] || [];
    const low = name.toLowerCase();
    return cats.some(c => (c?.name || "").toLowerCase() === low || (c?.slug || "").toLowerCase() === low);
  };
  ...
  function firstImgFromHTML(html) {
    ...
    if (ss) { const last = ss.split(",").map(s=>s.trim()).pop(); const url = last?.split(" ")?.[0]; if (url) return url; }
  ...
  // Home
  const HomeCache = { html:"", scrollY:0, hasData:false, search:"", page:1, totalPages:1 };
    ...
    async function load() {
      ...
      try {
        ...
        for (const p of posts) {
          ...
          let media = featuredImage(p) || firstImgFromHTML(p.content?.rendered) || firstImgFromHTML(p.excerpt?.rendered) || "";
          // For Newsmakers, try a thumbnail via first link (optional light handling)
          if (!media && inCategory(p, NEWSMAKERS_CAT)) {
            const link = (p.content?.rendered || "").match(/https?:\/\/\S+/)?.[0] || "";
            if (link) media = media || ""; // keep empty (GH Pages often blocks FB oEmbeds); card still clickable
          }
        ...
        HomeCache.html = app.innerHTML; HomeCache.scrollY = window.scrollY; HomeCache.hasData = true; HomeCache.page = page; HomeCache.totalPages = totalPages; HomeCache.search = search;
      } catch (e) {
  ...
  async function renderPost(id) {
    ...
    try {
      ...
      app.innerHTML = `
        <article class="post">
          ...
          ${tags.length ? `<div class="tags"><span style="margin-right:6px;">Tags:</span>${tags.map(t=>{
            const name=esc(t.name||"tag"); const slug=t.slug||""; const href = slug?`https://okobserver.org/tag/${slug}/`:"#";
      ...
      document.querySelectorAll(".post a[href]").forEach(a => { a.target="_blank"; a.rel="noopener"; });
    } catch (e) {
